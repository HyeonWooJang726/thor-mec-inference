...
def validate_run(output, c, k, frames, run_id):
    ...
    evidence = json.loads((output/'termination_evidence.json').read_text())
    termination = validate_termination(evidence)
    if evidence['mode'] != 'full_source' or frames != 1800 or not all(termination['eos_observed']):
        raise ValueError('natural full-source EOS contract required')
    if validation['EOS_validation'] != 'PASS':
        raise ValueError('source EOS validation failed')
    if termination != validation['termination'] or termination['validation'] != 'PASS':
        raise ValueError('full-source termination contract failure')
    if termination['eos_observed'] != validation['source_EOS']:
        raise ValueError('EOS observational metadata mismatch')
    for i in range(k):
        if evidence['per_stream'][i]['frame_ids'] != sorted(r['frame_id'] for r in records if r['stream_id'] == i):
            raise ValueError('termination ID evidence mismatch')
    ...
    for row, saved in zip(expected_rows, csv_rows):
        ...
        for field, value in saved.items():
            if field in ('C', 'K', 'run_id'):
                if value != str({'C': c, 'K': k, 'run_id': run_id}[field]):
                    raise ValueError('CSV run identity mismatch')
                continue
            if field in ('start_lag_ns', 'queue_wait_ns', 'local_latency_ns'):
                if int(value) != row[{'start_lag_ns': 'frame_start_lag_ns', 'queue_wait_ns': 'inference_queue_wait_ns', 'local_latency_ns': 'e2e_ns'}[field]]:
                    raise ValueError('CSV duration alias mismatch')
                continue
    ...
    return summary, {
        ...
        **validate_timing(records), 'negative_waiting_depth_events': sum(e['depth'] < 0 for e in events),
        ...
        'EOS': validation['EOS_validation'], 'termination': termination, 'concurrency': conc,
    }
