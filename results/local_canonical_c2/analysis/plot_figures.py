...
def main():
 ...
 def panel_labels(fig,axes,*,component_legend=False):
  fig.subplots_adjust(bottom=.36 if component_legend else .23,wspace=.3)
  for letter,ax in zip('abcd',axes):
   bb=ax.get_position();fig.text((bb.x0+bb.x1)/2,.18 if component_legend else .035,f'({letter})',ha='center',va='bottom',fontsize=11)
 ...
 for ax,K in zip(axes,groups):
  ...
  ax.set_title('Independent y scale' if len(groups)>1 else '')
 fig.legend(labels,loc='lower center',bbox_to_anchor=(.5,.015),ncol=4,frameon=False,fontsize=9);fig.subplots_adjust(top=.88)
 panel_labels(fig,axes,component_legend=True);save(fig,'figure_2_latency_decomposition','Each stacked height is the mean of five run-level component means. All components are per-frame wall-clock latency, in ms; independent K7 axis used only for readability if necessary. Mean components add exactly; component percentiles are never added.')
 ...
 axes[0].set_xticks(x,[f'K{k}' for k in pair]);axes[0].set_ylabel('Within-period ready-time span (ms)');axes[0].legend(fontsize=8);axes[0].set_title('Arrival-to-ready dispersion')
 ...
 axes[1].set_xticks(x,names,fontsize=8);axes[1].set_ylabel('Periods with carry-over (%)');axes[1].set_ylim(0,108);axes[1].legend(fontsize=8);axes[1].set_title('Waiting and active are distinct')
 ...
 axes[2].set_xlabel('Actual time relative to t0 (s)');axes[2].set_ylabel('Waiting queue: 1 s time mean');axes[2].set_ylim(bottom=0);axes[2].legend(fontsize=8);axes[2].set_title('All five runs; startup included')
 panel_labels(fig,axes);save(fig,'figure_3_temporal_queue',f'Observed transition K{pair[0]}→K{pair[1]}. Ready span is max(r)-min(r) for K frames in one logical period. Boundary is t0+floor((j+1)*1e9/30); counts earlier logical IDs only. Waiting r<=t<s excludes active s<=t<c. Next first-ready checks all prior unfinished work, including not-yet-ready work. Boundary fractions use 1800 periods; next-first-ready fractions 1799. Bars: five run-level mean ± sample SD. Right: separate exact event-integrated one-second queue traces, never sampled depth. No sum of overlapping service times is interpreted as capacity.')
 fig,axes=plt.subplots(1,2,figsize=(10,5.2))
 err(axes[0],'deadline_miss_pct','Deadline miss',red);axes[0].set_ylabel('Candidate deadline misses (%)');axes[0].set_ylim(-3,105);axes[0].set_title('QoS degradation with concurrent inference')
 err(axes[1],'queue_wait_mean_ms','Queue wait',red);err(axes[1],'inference_mean_ms','Inference service',green,marker='s')
 axes[1].set_ylabel('Mean per-frame duration (ms, log scale)');axes[1].set_yscale('log');axes[1].set_title('Queueing versus service duration')
