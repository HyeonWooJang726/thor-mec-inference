...
from core import *
from analyze import numbers

def aggregate():
 state=json.loads((ROOT/'campaign_status.json').read_text());passed=[r for r in state['rows'] if r['status']=='PASS' and r['stage']=='primary'];runs=[];windows=[];trans=[];seconds=[]
 for r in passed:
  d=ROOT/r['acquisition_id'];runs.append(json.loads((d/'summary.json').read_text()));windows+=numbers(d/'window_summary.csv');trans+=numbers(d/'transition_summary.csv');seconds+=numbers(d/'state_seconds.csv')
