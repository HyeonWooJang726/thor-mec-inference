...
def run(row):
 out=ROOT/row['acquisition_id'];frames=numbers(out/'per_frame.csv');events=numbers(out/'events.csv');term=json.loads((out/'termination.json').read_text());plan=Trace(row['trace'],row['duration_s'])
 checks=[]
 def check(name,value):
  checks.append(dict(check=name,PASS=bool(value)))
  if not value:raise AssertionError(name)
 try:
  check('actual_child_exit_zero',json.loads((out/'exit.json').read_text())['exit_code']==0)
 ...
 assert A==Q==B==U==0
 result=dict(**row,**metrics(frames),completed_frames=len(frames),expected_frames=len(plan.jobs),integrity='PASS',observed_max_A=maxA,
