...
R=Path(__file__).resolve().parents[2];D=R/'results/local_canonical_c2'
sys.path.insert(0,str(R/'scripts/common'))
from script_paths import configure,script_path
configure()
sys.path[:0]=[str(D/'code'),str(D/'analysis'),str(R/'scripts')]

def main():
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--output',type=Path,required=True);args=parser.parse_args()
 existing=unittest.TextTestRunner(verbosity=1).run(unittest.TestLoader().discover(str(R/'scripts/local'),pattern='test_local_*.py'))
 concurrency=unittest.TextTestRunner(verbosity=1).run(unittest.TestLoader().discover(str(R/'scripts/concurrency'),pattern='test_local_*.py'))
 ...
 tests=[existing,concurrency,current,events,historical];assert all(r.wasSuccessful() for r in tests)
 modules=['analyze_local_concurrency_formal_control','local_concurrency_control_metrics','local_concurrency_tensorrt','local_concurrency_validation','profile_local_concurrency_control','run_local_concurrency_control','run_local_concurrency_formal_control','run_local_concurrency_trtexec_probe','profile_fullsource','screening_tensorrt','screening_validation','screening_metrics','termination_contract','run_fullsource','analyze','event_analysis','temporal']
 for name in modules:importlib.import_module(name)
 paths=list((D/'code').glob('*.py'))+list((D/'analysis').glob('*.py'))+list((R/'scripts').rglob('*.py'))+[D/'verify_checkpoint.py',D/'verify_formal.py',D/'run_regressions.py']
